...
export async function POST(request: NextRequest) {
  try {
    ...
    const { pubKeyX, pubKeyY, signer, topicId, amount, signature } = body

    // Validate input
    if (!pubKeyX || !pubKeyY || !signer || topicId === undefined || !amount || !signature) {
      return NextResponse.json(
        { error: 'Missing required fields: pubKeyX, pubKeyY, signer, topicId, amount, signature' },
        { status: 400 }
      )
    }
